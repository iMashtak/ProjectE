...
--* use /libs/Controls/basic/button.lua
...
local function Initialize(_, addOnName)
    ...
    local button = Controls.button("E_BUTTON", layout, {
        hidden = Ref(false),
        anchors = Ref({
            { point = TOPLEFT,     target = layout.element, relativePoint = CENTER,     offsetX = 0, offsetY = 0 },
        })
    })
